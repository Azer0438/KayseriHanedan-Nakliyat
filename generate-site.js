...
function navMarkup() {
  return nav.map(([label, href, children]) => {
    ...
    return `<li class="has-dropdown"><a href="${href}">${label}<span aria-hidden="true">⌄</span></a><ul>${children.map(([childLabel, childHref]) => `<li><a href="${childHref}">${childLabel}</a></li>`).join("")}</ul></li>`;
  }).join("");
}
  ...
  <header class="site-header">
    <div class="topbar">
      <div class="container topbar-inner">
        <span>${icon("location")} ${address}</span>
    ...
    <div class="nav-shell">
      <div class="container nav-inner">
        ...
        <div class="header-actions">
          <a class="call-pill" href="tel:${secondaryTel}"><span>Randevu / Bilgi</span><strong>${secondaryPhone}</strong></a>
          <button class="menu-toggle" type="button" aria-expanded="false" aria-controls="mobileNav"><span></span><span></span><span></span></button>
        </div>
      </div>
...
.site-header .container { width: min(1500px, calc(100% - 60px)); }
...
.topbar { background: transparent; color: #fff; font-weight: 800; font-size: 13px; border-bottom: 1px solid rgba(255,255,255,.16); }
.topbar-inner { min-height: 36px; display: flex; align-items: center; gap: 34px; }
...
.nav-inner { min-height: 92px; display: flex; align-items: center; gap: 38px; }
.brand { display: inline-flex; align-items: center; min-width: 430px; }
.brand-logo { width: 340px; height: auto; display: block; }
.main-nav { flex: 1; border-left: 1px solid rgba(255,255,255,.2); padding-left: 38px; }
.main-nav ul, .mobile-nav ul { list-style: none; padding: 0; margin: 0; }
.main-nav > ul { display: flex; justify-content: flex-end; align-items: center; gap: 8px; }
.main-nav a { display: flex; align-items: center; gap: 7px; padding: 32px 8px; font-size: 14px; font-weight: 900; text-transform: uppercase; }
...
.header-actions { display: flex; align-items: center; gap: 12px; min-width: 226px; justify-content: flex-end; border-left: 1px solid rgba(255,255,255,.2); padding-left: 30px; }
.call-pill { display: grid; gap: 2px; }
.call-pill span { font-size: 12px; opacity: .85; }
.call-pill strong { font-size: 18px; line-height: 1; }
.menu-toggle { display: none; width: 58px; height: 58px; border: 0; background: #101010; padding: 16px; cursor: pointer; }
.menu-toggle span { display: block; height: 2px; margin: 6px 0; background: #fff; }
.mobile-nav { display: none; background: #fff; color: var(--ink); }
...
@media (max-width: 1100px) {
  ...
  .header-actions { min-width: auto; border-left: 0; padding-left: 0; }
  .menu-toggle { display: block; }
  .call-pill { display: none; }
  .mobile-nav.open { display: block; }
  .mobile-nav ul { padding: 14px 22px 24px; }
  .mobile-nav a { display: block; padding: 11px 0; font-weight: 850; }
  .mobile-nav .has-dropdown ul { display: grid; padding-left: 18px; color: var(--muted); }
